...
function main(): void {
  ...
  const skills = readJson("skills.json") as any[];
  ...
  for (const s of skills) {
    if (!employeeIds.has(s.employeeId)) throw new Error(`skills.json entry references missing employeeId ${s.employeeId}`);
  }
  ...
  const run = db.transaction(() => {
    const insertEmployee = db.prepare(`
      INSERT INTO employees (id, name, email, role, experience, team, primary_skill, secondary_skill, projects_json, profile_image, status, manager_id)
      VALUES (@id, @name, @email, @role, @experience, @team, @primarySkill, @secondarySkill, @projectsJson, @profileImage, @status, @managerId)
    `);
    for (const e of employees) {
      insertEmployee.run({
        id: e.id,
        ...
        secondarySkill: e.secondarySkill,
    ...
    const insertProject = db.prepare(`
      INSERT INTO projects (id, name, client, program, manager, tech_lead, technology_json, stage, status, ai_adoption, members_json, start_date, end_date)
      VALUES (@id, @name, @client, @program, @manager, @techLead, @technologyJson, @stage, @status, @aiAdoption, @membersJson, @startDate, @endDate)
    `);
    for (const p of projects) {
      insertProject.run({
        id: p.id,
    ...
    const insertSkill = db.prepare(`
      INSERT INTO skills (employee_id, Magento, PHP, React, JavaScript, GraphQL, MySQL, Docker, Git, Claude, ChatGPT, GitHubCopilot, Cursor, PromptEngineering)
      VALUES (@employeeId, @Magento, @PHP, @React, @JavaScript, @GraphQL, @MySQL, @Docker, @Git, @Claude, @ChatGPT, @GitHubCopilot, @Cursor, @PromptEngineering)
    `);
    for (const s of skills) {
      insertSkill.run(s);
    }
    ...
    const insertCalendarEvent = db.prepare(`
      INSERT INTO calendar_events (id, employee_id, title, description, event_type, start, end, time_zone, organizer, attendees_json, location, outlook_event_id, created_by, linked_task_id, linked_poc_id, block_group_id)
      VALUES (@id, @employeeId, @title, @description, @eventType, @start, @end, @timeZone, @organizer, @attendeesJson, @location, @outlookEventId, @createdBy, @linkedTaskId, @linkedPocId, @blockGroupId)
    `);
    for (const c of calendarEvents) {
  ...
  assertCount("skills", skills.length, count("skills"));
